            ...
            <div class="modal-header text-center" style="background-color: lightblue;">
            	<div class="panel-heading">
					<div class="col-lg-8 text-left">
						<span class="modal-title w-80"><i class="text-primary fa fa-pen"></i> Add More Hospitals</span>
					</div>
					<div class="col-lg-3">
					...
					<div class="col-lg-1">
						<button type="button" class="close sbmt-btn" id="cancelBtn" style="opacity: 1; background-color: red; padding: 5px 10px; border-radius: 50%;" data-dismiss="modal" aria-label="Close">
		    ...
		    <form action="{{ route('hospital.store') }}" method="post" enctype="multipart/form-data">
		     {{ csrf_field() }}
		        ...
		        <div class="modal-footer d-flex justify-content-center">
		            <button class="btn btn-unique btn-success btn-sm btn-block" id="addBtn"><i class="fas fa-paper-plane ml-1"></i> ADD</button>
		        </div>
...
<div class="text-center" >
    	...
    	@if ($errors->any())
    		<a style="" href="" class="btn btn-danger btn-rounded mb-4 btn-lg" data-toggle="modal" data-target="#modalContactForm">FORM NOT SENT. THERE ARE ( {{ $errors->count() }} ) ERRORS. ADDRESSED THEM.</a>
    	@else
    		<a style="" id="addHsp" href="" class="btn btn-success btn-rounded mb-4 btn-lg" data-toggle="modal" data-target="#modalContactForm">ADD HOSPITAL</a>
    	@endif
</div>

<script>
	{{-- Hide Enquiry modal conflicting add hospital modal button --}}
	document.getElementById('addHsp').addEventListener('mouseover', hideModalEnquiry);
	function hideModalEnquiry() {
		var modalEnq = document.querySelectorAll('#modalEnquiry');//.style.display='none';
		var i;
	    for (i = 0; i < modalEnq.length; i++) {
	        modalEnq[i].style.display = "none";
	    }
	}
{{-- Show Enquiry modal conflicting add hospital modal button --}}
	document.getElementById('cancelBtn').addEventListener('click', showModalEnquiry);
	function showModalEnquiry() {
		var modalEnq = document.querySelectorAll('#modalEnquiry');//.style.display='inline-block';
		var i;
	    for (i = 0; i < modalEnq.length; i++) {
	        modalEnq[i].style.display = "inline-block";
	    }
	}
</script>